...
public abstract class Debug {
    ...
    public static void msg(Message msg) {
        log(DEFAULT_MESSAGE_LABEL, msg, true, null);
    }

    public static void msg(Message msg, Object... extras) {
        log(DEFAULT_MESSAGE_LABEL, msg, true, extras, false);
    }

    public static void msg(String label, Message msg, Object... extras) {
        log(label, msg, true, extras, false);
    }
    ...
    public static void stackTrace(String label, Throwable t, Object... extras) {
        ...
        if (m == null) {
            log(label, grow("Stack Trace", extras));
        }
        else {
            log(label, grow(t.getMessage(), extras));
        }
    ...
    public static void log(String label, Object... extras) {
        if (PAUSE) { return; }
        if (extras == null || extras.length == 0) {
            log(label, null, false, null, false);
        }
        else if (extras[0] instanceof NatsMessage) {
            log(label, (NatsMessage)extras[0], true, extras, true);
        }
        else {
            log(label, null, false, extras, false);
        }
    }

    public static void log(String label, Message msg, boolean forMsg, Object[] extras, boolean skipFirst) {
        if (PAUSE) { return; }
        String start;
    ...
    public static String format3Right(Number n, int width) {
        return String.format("%" + width + "s", format3(n, true));
    }
